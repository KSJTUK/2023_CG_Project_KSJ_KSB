...
{
	vec3 textureDiffuse = notextureID == 1 ? vec3(0.5f) : vec3(texture(meterials.diffuse, vs_out_tex));
...
{
	vec3 ambient = light.ambient * vec3(texture(meterials.diffuse, vs_out_tex));
	...
	float diffuseN = max(dot(vNorm, lightDirection), 0.0f);
	vec3 diffuse = light.diffuse * (diffuseN * vec3(texture(meterials.diffuse, vs_out_tex)));
...
void main(void)
{
	vec3 resultColor = calcDirectionLighting(dirLight, vs_out_normal, viewPosition, fragPosition);

	if (notextureID == 1) {
		resultColor += notexturePointLighting(pointLight, vs_out_normal, viewPosition, fragPosition);
	} else {
		resultColor += calcPointLighting(pointLight, vs_out_normal, viewPosition, fragPosition);
	}
